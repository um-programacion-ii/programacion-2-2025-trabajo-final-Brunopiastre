...
public class VentaService {
    ...
    public VentaResponseDTO procesarVenta(VentaRequestDTO request, String userLogin) {
        ...
        // Obtener usuario (Corregido: uso de orElseThrow en lugar de get)
        User usuario = userRepository.findOneByLogin(userLogin)
            .orElseThrow(() -> new IllegalArgumentException("Usuario no encontrado: " + userLogin));

        // Obtener evento (Corregido: uso de orElseThrow en lugar de get)
        Evento evento = eventoRepository.findByEventoIdCatedra(request.getEventoId())
            .orElseThrow(() -> new IllegalArgumentException("Evento no encontrado: " + request.getEventoId()));
    ...
    public boolean reintentarVenta(Venta venta) {
        ...
        // Obtener evento para calcular precio (Corregido: uso de orElse(null) para validación)
        Evento evento = eventoRepository.findById(venta.getEventoId()).orElse(null);
        if (evento == null) {
